...
# 启动:先跑 migrate deploy(幂等),再 exec node 让 node 替换 sh 进程
# 配合 tini PID 1 让 SIGTERM 直达 fastify,优雅关闭
CMD ["sh", "-c", "pnpm prisma migrate deploy && exec node dist/server.js"]
