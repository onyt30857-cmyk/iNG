...
9. ~~死代码岛(spec-005 残留)~~ **已 2026-05-12 物理拆除**(commit `0db7226` Scope 1 + `fa8e0b7` Scope 2)。12 死代码岛源文件 + 5 测试 + 9 幽灵 prompt 工程台文件 + admin /prompts 整目录已删。`server.ts` 中 `sessionRoutes` import + register 也已清。M3.0 Item 1 Scope 1+2 完结。Scope 4(`ai/client.ts` 重构提炼 helper)2026-05-14 完成,callClaude/Stream 主体重复消除。
