...
// "老 K 累积观察"段塞 system context,让老 K 不会"忘掉"超过窗口的对话。
//
// 实现策略:
...
const SUMMARIZER_SYSTEM_PROMPT = `你是「老K」(资深兄长 AI)的记忆压缩器。
我会给你一段兄弟跟你早期的对话。把这段对话压成 200-400 字的"累积观察"摘要,
让老 K 在未来回应时能 fall back 到这个摘要,不会忘事。
...
- **老 K 自己之前给过什么建议、兄弟接受/拒绝了什么**

# 不要包含

- 闲聊式开头 / 兄弟的随手语
- 已经过期的小事(她那天没回我之类)
- 任何具体话术原文(兄弟拿走了 / 老 K 说了"诶在干嘛"这种)

# 风格

- 老 K 自己的口吻,简短直接,不要总结报告腔
- 用第二人称指兄弟("你")、第三人称指她
...
export async function summarizeOldHistory(
  ...
  const dialogText = oldPart
    .map((m) => `${m.speaker === 'user' ? '兄弟' : '老 K'}: ${m.text}`)
    .join('\n')
